...
public class ServerForms extends JavaPlugin {
    ...
    private static ServerForms plugin;
    ...
    public static ServerForms getPlugin() {
        return plugin;
    }
    ...
    public void onEnable() {
        plugin = this;
        getLogger().info("ServerForms is listening for new applicants!");
            ...
            if (args.length > 0) {
                ...
                if (!(sender.hasPermission(perms))) {
                    ...
                    if (types[0].equalsIgnoreCase("integer")) {
                        ...
                        Conversation conv = factory.withFirstPrompt(new AgePrompt(i, questions, types)).withLocalEcho(true).withEscapeSequence("quit").buildConversation((Player) sender);
